...
class ControlRound:
    ...
    def set_pair_list_random(tournament: Tournament) -> list:
        ...
        for match_number in range(0, int(len(player_list)/2)):
            ...
            player2 = [player_list[match_number + number_of_matches_per_round], 0]
    ...
    def set_pair_list_based_on_scores(self, program_file: ProgramData) -> list:
        ...
        # Shuffle players in order to randomly put together players with the same score
        random.shuffle(list(current_tournament_player_list.values()))
        # sort players by score
        player_list_sorted = sorted(current_tournament_player_list.values(), key=lambda x: x.score, reverse=True)
        ...
        # create match list
        match_list = []
        # each match entry will contain a group of 2 players taken in the order it's provided in the pairings list
    ...
    def set_match_score(self, program_file: ProgramData):
        ...
        while score_player1 != [0, 0.5, 1]:
            ...
            user_input = self.round_view.prompt_for_score_input(player1_national_chess_identifier)
    ...
    def is_last_round(program_file: ProgramData):
        ...
        return ongoing_tournament.current_round > ongoing_tournament.total_rounds
